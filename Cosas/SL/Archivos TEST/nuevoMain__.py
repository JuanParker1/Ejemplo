...
direccion = '10.52.104.154' #10.52.104.115
puerto = '30015' #30013
usuario = 'SYSTEM'
clave = 'Innovate2018'
    ...
    try:
        ...
        codigosSize = db.consultarFilas(
            'SELECT "U_SizeCode" FROM "' + dataBase + '"."@ARGNS_SIZE" WHERE "Code" = ' + "'" + db.consultaElemento(
                '"' + dataBase + '"."@ARGNS_SCALE"', 'U_SclCode', filaActual['Size Chart'], 'Code').strip() + "'")
        cuerpoCodigosSize = []
        ...
        for columnaUDF in columnasNuevas:
            if str(filaActual[columnaUDF]) != 'nan':
                udfs['U_' + columnaUDF.replace(" ", "_")] = filaActual[columnaUDF]
        ...
        cuerpo = {"Code": str(identificador),
                  ...
                  "U_ModGrp": db.consultaElemento('"' + dataBase + '"."@ARGNS_MODELGRP"', 'U_ProdLine',
